...
function setupMobileFeatures() {
    ...
    window.toggleMobileFilters = function(tabName) {
        const filters = document.querySelector(`#${tabName} .filters`);
        if (filters) {
            filters.classList.toggle('mobile-active');
            
            // Update button text based on state
            const button = document.querySelector(`#${tabName} .mobile-filter-toggle .btn`);
            if (button) {
    ...
    document.addEventListener('click', function(e) {
        if (!e.target.closest('.filters') && !e.target.closest('.mobile-filter-toggle')) {
            const activeFilters = document.querySelectorAll('.filters.mobile-active');
            activeFilters.forEach(filters => {
                filters.classList.remove('mobile-active');
                const button = filters.parentElement.querySelector('.mobile-filter-toggle .btn');
                if (button) {
                    button.innerHTML = '<i class="fas fa-filter"></i> Filters';
                }
            });
    ...
    // Mobile filter chip functionality
    window.applyMobileFilter = function(tabName, filterType, filterValue) {
        // Update the appropriate filter dropdown
        const filterSelect = document.querySelector(`#${tabName} #${tabName}${filterType.charAt(0).toUpperCase() + filterType.slice(1)}Filter`);
        if (filterSelect) {
        ...
        const chips = document.querySelectorAll(`#${tabName} .mobile-filter-chip`);
        chips.forEach(chip => {
        ...
        if (clickedChip.classList.contains('mobile-filter-chip')) {
            clickedChip.classList.add('active');
        }
    };
    
    window.clearMobileFilters = function(tabName) {
        ...
        // Clear search input
        const searchInput = document.querySelector(`#${tabName} .filters input[type="text"]`);
        if (searchInput) {
            searchInput.value = '';
        }
